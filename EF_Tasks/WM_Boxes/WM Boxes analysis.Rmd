...
wilcox.test(WM.boxes.valid.TT1$ProportionCorrectinTest ~ WM.boxes.valid.TT1$TestingLocation, paired = FALSE, alternative = "greater")
```
...
```
There is a significant effect of age (X2(1) = 0.239, p = .004) and testing location (X2(1) = 0.359, p < .001). With each standard deviation increase in age, the proportion correct increases by 0.04 (95% CI [0.01; 0.01]) percentage points. When moving from Edinburgh to Fife, the expected score for a child of mean age decreases by 0.10 (95% CI [0.04; 0.16]) percentage points.