...
wilcox.test(Prop_Switches$ProportionSwitches, mu = 0.2242245, alternative = "two.sided")
```
...
p + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(axis.text.x = element_blank()) + theme(axis.ticks.x = element_blank()) +  geom_hline(yintercept=0.2242245, linetype="dashed", color = "red") + geom_count()
```

Split by the three age groups:
```{r}
p1<-ggplot(data=Shelf.clean.test.T1, aes(x=AgeGroup_midtesting,y=ProportionSwitches)) +
...
```{r}
p1<- ggplot(Shelf.clean.test.T1, aes(x=AgeMonths_midtesting, y=ProportionSwitches)) + geom_point()+
 labs(x="Age in months",y="Proportion correct switches")
...
# In the test phase, can age and trial number and the interaction between age and trial number predict success?
```{r}
library(lme4)
...
Shelf.clean.test$z.Trial_nr=as.vector(scale(Shelf.clean.test$Trial_nr))#transform trial number to mean of 0 and SD of 1
mean(Shelf.clean.test$z.Trial_nr)#check whether it has worked
sd(Shelf.clean.test$z.Trial_nr)

#Full model
control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000))

res<-glmer(Success ~ z.age.midtesting + z.Trial_nr + z.age.midtesting:z.Trial_nr + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control = control)

null<-glmer(Success ~ 1 + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control=control)

anova(null, res, test="Chisq")
```
The full model explains the data significantly better than the null model only consisting of the intercept, X2(3) = 50.714, p < .001.

Effect of interaction
```{r}
res.nointer<-glmer(Success ~ z.age.midtesting + z.Trial_nr + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control=control)

anova(res.nointer, res, test="Chisq")

```
There is only a marginally significant interaction effect, X2(1) = 3.557, p = .059, so we remove it from the model.


```{r}
res<-glmer(Success ~ z.age.midtesting + z.Trial_nr + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control=control)

anova(null, res, test="Chisq")

summary(res)
```
The full model explains the data significantly better than the null model only consisting of the intercept, X2(2) = 47.157, p < .001.

Effect of trial number
```{r}
res.notrial<-glmer(Success ~ z.age.midtesting + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control=control)

anova(res.notrial, res, test="Chisq")
```
Trial number has a significant effect, X2(1) = 28.231, p < .001.

Effect of age
```{r}
res.noage<-glmer(Success ~ z.Trial_nr + (1+z.Trial_nr|ID), data=Shelf.clean.test, family = binomial, control=control)

anova(res.noage, res, test="Chisq")
```
Age has a significant effect, X2(1) = 16.943, p < .001.
...
anova(null, res, test="Chisq")
```
The full model explains the data significantly better than the null model only consisting of the intercept, X2(4) = 39.015, p < .001.
...
```
There is no significant interaction effect, X2(1) = 0.968, p = .325, so we remove it from the model.
...
summary(res)
```
The full model explains the data significantly better than the null model only consisting of the intercept, X2(3) = 38.047, p < .001.
...
anova(res.notrial, res, test="Chisq")
```
Trial number has a significant effect, X2(1) = 21.391, p < .001.
...
anova(res.noage, res, test="Chisq")
```
Age has a significant effect, X2(1) = 14.889, p < .001.
...
```
There is no effect of the BPVS score, X2(1) = 0.00, p = .992.
