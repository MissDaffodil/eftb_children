...
t.test(ShiftingBoxes.SD.Trial1.valid$Proportion_Correct, mu = 0.5, alternative = "two.sided")
...
### Boxplots
Mean proportion correct, across ages
```{r}
library(ggplot2)
...
# CD: All children who started that phase, independent of whether or not they reached criterion in SD 
## How many children started the task?
```{r}
...
t.test(ShiftingBoxes.CD.Trial1.valid$Proportion_Correct, mu = 0.5, alternative = "two.sided")
...
threeandfour <- subset(ShiftingBoxes.CD.Trial1.valid, AgeGroup_midtesting == "3" | AgeGroup_midtesting == "4")

wilcox.test(threeandfour$Proportion_Correct~ threeandfour$AgeGroup_midtesting, alternative = "less")
...
t.test(ShiftingBoxes.CD.Trial1.valid$Proportion_Correct ~ ShiftingBoxes.CD.Trial1.valid$TestingLocation, alternative = "two.sided")
```
...
```{r}
res.nointer<-glmer(success ~ z.age.midtesting + z.Trial_nr + (1|ID), data=ShiftingBoxes.CD.valid, family = binomial, control = contr)#converges, no singular fit warning

anova(null, res.nointer, test="Chisq")
```
Age and trial number alone do not explain the data significantly better than the null model, X2(2) = 4.303, p = .116.



# Considering BPVS score: What is the effect of age and trial number on performance in CD (all children)?
```{r}
library(lme4)
ShiftingBoxes.CD.valid.BPVS <- subset(ShiftingBoxes.CD.valid, BPVS_Score != "NA")
#we z-transform age and trial number, as we are including them into an interaction
ShiftingBoxes.CD.valid.BPVS$z.age.midtesting=as.vector(scale(ShiftingBoxes.CD.valid.BPVS$AgeMonths_midtesting))#transform age to mean of 0 and SD of 1
mean(ShiftingBoxes.CD.valid.BPVS$z.age.midtesting)#check whether it has worked
sd(ShiftingBoxes.CD.valid.BPVS$z.age.midtesting)

ShiftingBoxes.CD.valid.BPVS$z.Trial_nr=as.vector(scale(ShiftingBoxes.CD.valid.BPVS$Trial_nr))#transform trial number to mean of 0 and SD of 1
mean(ShiftingBoxes.CD.valid.BPVS$z.Trial_nr)#check whether it has worked
sd(ShiftingBoxes.CD.valid.BPVS$z.Trial_nr)

ShiftingBoxes.CD.valid.BPVS$z.BPVS_Score=as.vector(scale(ShiftingBoxes.CD.valid.BPVS$BPVS_Score))

#Full model
contr<-glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=10000000))

res<-glmer(success ~ z.BPVS_Score + z.age.midtesting + z.Trial_nr + z.age.midtesting:z.Trial_nr + (1+z.Trial_nr|ID), data=ShiftingBoxes.CD.valid.BPVS, family = binomial, control = contr)#singular fit
```
The initial full model yields a singular fit. This is because the variance for the random slope of trial on ID is basically 0, so we remove this term from the model.
```{r}
res<-glmer(success ~ z.BPVS_Score + z.age.midtesting + z.Trial_nr + z.age.midtesting:z.Trial_nr + (1|ID), data=ShiftingBoxes.CD.valid.BPVS, family = binomial, control = contr)#converges, no singular fit warning

#null model
null<-glmer(success ~ 1 + (1|ID), data=ShiftingBoxes.CD.valid.BPVS, family = binomial, control = contr)

anova(null, res, test="Chisq")
```

Together, BPVS score, age, trial number and the interaction between age and trial number do not explain the data significantly better than a null model containing only the intercept, X2(4) = 6.205, p = .184.

Removal of interaction term
```{r}
res.nointer<-glmer(success ~ z.BPVS_Score + z.age.midtesting + z.Trial_nr + (1|ID), data=ShiftingBoxes.CD.valid.BPVS, family = binomial, control = contr)#converges, no singular fit warning

anova(null, res.nointer, test="Chisq")
```
Age, BOVS score, and trial number alone do not explain the data significantly better than the null model, X2(3) = 3.764, p = .288.
...
# What is the effect of age and trial number on performance in CD (only children who reached criterion in SD)?
```{r}
library(lme4)
#we z-transform age and trial number, as we are including them into an interaction
ShiftingBoxes.CD.valid$z.age.midtesting=as.vector(scale(ShiftingBoxes.CD.valid$AgeMonths_midtesting))#transform age to mean of 0 and SD of 1
