...
Grid$TriedTransparentBox <- as.numeric(as.character(Grid$TriedTransparentBox))
str(Grid)
```
...
```
There are 64 girls and 62 boys.
...
## How many children chose the transparent box in the first search?
```{r}
sum(Grid.valid.T1$TriedTransparentBox)

mean(Grid.valid.T1$TriedTransparentBox,na.rm = T)

shapiro.test(Grid.valid.T1$TriedTransparentBox)
wilcox.test(Grid.valid.T1$TriedTransparentBox, mu = 0.5, alternative = "two.sided")
t.test(Grid.valid.T1$TriedTransparentBox, mu = 0.5, alternative = "two.sided")

binom.test(127, 155, p = 0.5,
...
t.test(Grid.valid.T1$ProportionCorrectOutOfFirstSixUniqueSearches, mu = 0.5, alternative = "two.sided")
...
t.test(Edi$ProportionCorrectOutOfFirstSixUniqueSearches, Fife$ProportionCorrectOutOfFirstSixUniqueSearches, alternative = "two.sided")
...
### Considering BPVS score: Can age predict children's success?
```{r}
Grid.valid.T1.BPVS <- subset(Grid.valid.T1, BPVS_Score != "NA")

Grid.valid.T1.BPVS$z.age=as.vector(scale(Grid.valid.T1.BPVS$Midtesting_age_months ))#transform age to mean of 0 and SD of 1
mean(Grid.valid.T1.BPVS$z.age)#check whether it has worked
sd(Grid.valid.T1.BPVS$z.age)

res<-lm(ProportionCorrectOutOfFirstSixUniqueSearches ~ z.age + BPVS_Score, data=Grid.valid.T1.BPVS)
null<-lm(ProportionCorrectOutOfFirstSixUniqueSearches ~ 1, data=Grid.valid.T1.BPVS)

anova(null, res, test="Chisq")#significant
```
Age and BPVS score together cannot predict the proportion corect ouf of the first 6 searches, X2(2) = 0.124, p = .413.
