...
## Distance
```{r}
mean(warm.up$distance_c01)
sd(warm.up$distance_c01)
min(warm.up$distance_c01)
max(warm.up$distance_c01)

#recode the distance variable to a range of 0-1
library(scales)
warm.up$distance_newrange <- rescale(warm.up$distance_c01, to=c(0, 1), from=c(0, 3.605551))

#revert scale, so that high numbers mean closeness to reward
warm.up$revertdistance_newrange <- 1-warm.up$distance_newrange

meandistancewarmup.revertnewrange<- aggregate(revertdistance_newrange ~ ID, warm.up, mean)

mean(meandistancewarmup.revertnewrange$revertdistance_newrange)
sd(meandistancewarmup.revertnewrange$revertdistance_newrange)
min(meandistancewarmup.revertnewrange$revertdistance_newrange)
max(meandistancewarmup.revertnewrange$revertdistance_newrange)

#rescale chance value
chance_grid_allcells <- rescale(1.954, to=c(0, 1), from=c(3.605551, 0))
chance_grid_allcells
#0.46

t.test(meandistancewarmup.revertnewrange$revertdistance_newrange, mu=0.46, alternative = "two.sided")
```
Mean closeness to reward in the warm-up trials was 0.94 (SD = 0.09, range 0.56-1).
...
```{r}
library(scales)
test.trials$distance_newrange <- rescale(test.trials$distance_c01, to=c(0, 1), from=c(0, 3.605551))
...
# chance value for the new scale
...
p + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(axis.text.x = element_blank()) + theme(axis.ticks.x = element_blank()) + geom_count() +  geom_hline(yintercept=0.5376574, linetype="dashed", color = "red")

```

## Rescaled histogram
```{r}
hist(meandistance.revertnewrange$revertdistance_newrange,
     main="Histogram for Mean distance between reward and first choice in test trials", 
     xlab="Mean distance", 
     ylab="Number of children",
     border="black", 
...
## rescaled Mean
```{r}
mean(meandistance.revertnewrange$revertdistance_newrange)
sd(meandistance.revertnewrange$revertdistance_newrange)
min(meandistance.revertnewrange$revertdistance_newrange)
max(meandistance.revertnewrange$revertdistance_newrange)
```
...
t.test(meandistance.revertnewrange$revertdistance_newrange, mu=0.46, alternative = "two.sided")
```
...
t.test(meandistance.revertnewrange$revertdistance_newrange, mu=0.54, alternative = "two.sided")
```
...
threeandfour <- subset(Grid.short, AgeGroup_midtesting == "3" | AgeGroup_midtesting == "4")
t.test(threeandfour$distance ~ threeandfour$AgeGroup_midtesting, alternative = "two.sided")

#rescaled
library(scales)
Grid.short$distance_newrange <- rescale(Grid.short$distance, to=c(0, 1), from=c(0, 3.605551))
#revert scale, so that high numbers mean closeness to reward
Grid.short$distance_newrange <- 1-Grid.short$distance_newrange

tapply(Grid.short$distance_newrange, Grid.short$AgeGroup_midtesting, mean)
tapply(Grid.short$distance_newrange, Grid.short$AgeGroup_midtesting, sd)
tapply(Grid.short$distance_newrange, Grid.short$AgeGroup_midtesting, min)
tapply(Grid.short$distance_newrange, Grid.short$AgeGroup_midtesting, max)
...
Four-year-olds perform significantly better than 3-year-olds, t(113,79) = 2.585, p = .011.
...
p1  +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  geom_count(colour = "black", position = position_jitterdodge(0,0,1), alpha = 0.5)+
  scale_fill_discrete(name = "Age group midtesting", labels = c("3 years, n = 55", "4 years, n = 61", "5 years, n = 10", "6 years, n = 1")) + geom_hline(yintercept=0.5376574, linetype="dashed", color = "red")
```
...
#rescaled
tapply(Grid.short$distance_newrange, Grid.short$Mediansplit_midtesting_entiresample, mean)
tapply(Grid.short$distance_newrange, Grid.short$Mediansplit_midtesting_entiresample, sd)
tapply(Grid.short$distance_newrange, Grid.short$Mediansplit_midtesting_entiresample, min)
tapply(Grid.short$distance_newrange, Grid.short$Mediansplit_midtesting_entiresample, max)

t.test(Grid.short$distance_newrange ~ Grid.short$Mediansplit_midtesting_entiresample, paired = FALSE, alternative = "greater")
```
...
p1  +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(axis.text.x = element_blank()) + theme(axis.ticks.x = element_blank()) +
  geom_point(pch = 21, position = position_jitterdodge(0,0,1.5)) +
  scale_fill_discrete(name = "Age group midtesting (median split)", labels = c("young (36-49 months), n = 65", "old (50-72 months), n = 62")) + geom_hline(yintercept=0.5376574, linetype="dashed", color = "red")
```
...
## Plot distance against age as continuous variable - new scale
```{r}
p1<- ggplot(Grid.short, aes(x=AgeMonths_midtesting, y=revertdistance_newrange)) + geom_point()

p1 +  geom_hline(yintercept=0.5376574, linetype="dashed", color = "red") +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```
...
t.test(Grid.short$distance ~ Grid.short$TestingLocation, paired = FALSE, alternative = "two.sided")


#rescaled
tapply(Grid.short$distance_newrange, Grid.short$TestingLocation, mean)
tapply(Grid.short$distance_newrange, Grid.short$TestingLocation, sd)
tapply(Grid.short$distance_newrange, Grid.short$TestingLocation, min)
tapply(Grid.short$distance_newrange, Grid.short$TestingLocation, max)


t.test(Grid.short$distance_newrange ~ Grid.short$TestingLocation, paired = FALSE, alternative = "two.sided")
```
...
p1 +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(axis.text.x = element_blank()) + theme(axis.ticks.x = element_blank()) +
  geom_point(pch = 21, position = position_jitterdodge(0,0,1.5)) +
  scale_fill_discrete(name = "Testing location", labels = c("Edinburgh, n = 58", "Fife, n = 69")) + geom_hline(yintercept=0.5376574, linetype="dashed", color = "red")
```
...
# Can the distance be predicted by age and testing location?
```{r}
library(lme4)

test.trials$z.age=as.vector(scale(test.trials$AgeMonths_midtesting))#transform age to mean of 0 and SD of 1
mean(test.trials$z.age)#check whether it has worked
sd(test.trials$z.age)

contr<-lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=10000000))
dist<-lmer(distance_c01 ~ z.age + Trial + TestingLocation + (1+Trial|ID), data=test.trials, REML=FALSE, control=contr)#singular fit

#remove correlation
dist<-lmer(distance_c01 ~ z.age + Trial + TestingLocation + (1|ID) + (0+Trial|ID), data=test.trials, REML=FALSE, control=contr)#singular fit
summary(dist)
```
The variance for the random slope of trial on ID is effectively 0, so we exclude it from the model.
```{r}
dist<-lmer(distance_c01 ~ z.age + Trial + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)

null.dist<-lmer(distance_c01 ~ 1 + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
**Effect of age**
```{r}
dist.noage<-lmer(distance_c01 ~ Trial + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
```{r}
dist.notrial<-lmer(distance_c01 ~ z.age + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
```{r}
dist.noloc<-lmer(distance_c01 ~ z.age + Trial + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
dist<-lmer(distance_c01 ~ z.age + z.Trial + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
ranef.diagn.plot(dist)

#collinearity
xres=lm(distance_c01 ~ z.age + Trial + TestingLocation, data=test.trials)

library(car)
vif(xres)

source("glmm_stability.r")
m.stab=glmm.model.stab(model.res=dist, contr=contr)
m.stab$summary
```
...
```{r}
library(lme4)
test.trials$distance_newrange <- rescale(test.trials$distance_c01, to=c(0, 1), from=c(3.605551, 0))

dist<-lmer(distance_newrange ~ z.age + Trial + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)

null.dist<-lmer(distance_newrange ~ 1 + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
dist<-lmer(distance_newrange ~ z.age + z.Trial + TestingLocation + (1|ID), data=test.trials, REML=FALSE, control=contr)
...
ggplot(test.trials, aes(x=plot.xvals, y=boot.res$ci.predicted$fitted)) +
  geom_jitter(aes(x=Trial, y=distance_newrange, colour=Age_group))+ 
  geom_ribbon(data=boot.res$ci.predicted, aes(x = plot.xvals, ymin=boot.res$ci.predicted$lower.cl, ymax=boot.res$ci.predicted$upper.cl), fill="grey", alpha=0.5)+
geom_line(data=boot.res$ci.predicted, aes(x = plot.xvals, y=boot.res$ci.predicted$fitted), lty=2)+
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Trial number")+
...
```{r}
xx=aggregate(x=test.trials$distance_newrange, by=test.trials[, c("TestingLocation", "Trial")],
FUN=mean)

xx$N=aggregate(x=test.trials$distance_newrange, by=test.trials[, c("TestingLocation", "Trial")],
FUN=length)$x

par(mar=c(3, 3, 0.2, 0.2), mgp=c(1.7, 0.3, 0), las=1, tcl=-0.15)

plot(x=test.trials$Trial, y=test.trials$distance_c01, pch=19, las=1, ylim=c(0, 1),
xlab="Trial number", ylab="distance", cex=0.2*sqrt(xx$N))
...
ggplot(test.trials, aes(x=plot.xvals, y=boot.res$ci.predicted$fitted)) +
  geom_jitter(aes(x=Trial, y=distance_newrange, colour=Age_mediansplit))+ 
  geom_ribbon(data=boot.res$ci.predicted, aes(x = plot.xvals, ymin=boot.res$ci.predicted$lower.cl, ymax=boot.res$ci.predicted$upper.cl), fill="grey", alpha=0.5)+
geom_line(data=boot.res$ci.predicted, aes(x = plot.xvals, y=boot.res$ci.predicted$fitted), lty=2)+
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Trial number")+
...
# Considering BPVS: Can the distance be predicted by age and testing location?
```{r}
library(lme4)

#select only those children who have  BPVS score
BPVS <- subset(test.trials, BPVS_Score != "NA")

BPVS$z.age=as.vector(scale(BPVS$AgeMonths_midtesting))#transform age to mean of 0 and SD of 1
mean(BPVS$z.age)#check whether it has worked
sd(BPVS$z.age)

contr<-lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=10000000))
dist<-lmer(distance_c01 ~ BPVS_Score+ z.age + Trial + TestingLocation + (1+Trial|ID), data=BPVS, REML=FALSE, control=contr)#singular fit

#remove correlation
dist<-lmer(distance_c01 ~ BPVS_Score+ z.age + Trial + TestingLocation + (1|ID) + (0+Trial|ID), data=BPVS, REML=FALSE, control=contr)#singular fit
summary(dist)
```
The variance for the random slope of trial on ID is effectively 0, so we exclude it from the model.
```{r}
dist<-lmer(distance_c01 ~ BPVS_Score + z.age + Trial + TestingLocation + (1|ID), data=BPVS, REML=FALSE, control=contr)

null.dist<-lmer(distance_c01 ~ 1 + (1|ID), data=BPVS, REML=FALSE, control=contr)

anova(null.dist, dist, test="Chisq")#significant
summary(dist)
```
Together, BPVS_Score, age, trial number, and testing location explain the data significantly better than the null model, X(4) = 26.221, p < .001.

**Effect of BPVS score**
```{r}
dist.nobpvs<-lmer(distance_c01 ~ z.age + Trial + TestingLocation + (1|ID), data=BPVS, REML=FALSE, control=contr)

anova(dist.nobpvs, dist, test="Chisq")#significant
```
There is a significant effect of BPVS score on children's success, X2(1) = 6.64, p = .010.

**Effect of age**
```{r}
dist.noage<-lmer(distance_c01 ~ BPVS_Score + Trial + TestingLocation + (1|ID), data=BPVS, REML=FALSE, control=contr)

anova(dist.noage, dist, test="Chisq")#significant
```
Age has a significant effect on distance, X2(1) = 6.053, p = .014.

**Effect of trial number**
```{r}
dist.notrial<-lmer(distance_c01 ~ BPVS_Score+z.age + TestingLocation + (1|ID), data=BPVS, REML=FALSE, control=contr)

anova(dist.notrial, dist, test="Chisq")#significant
```
Trial number has a significant effect on distance, X2(1) = 5.232, p = .022.

**Effect of testing location**
```{r}
dist.noloc<-lmer(distance_c01 ~ BPVS_Score + z.age + Trial + (1|ID), data=BPVS, REML=FALSE, control=contr)

anova(dist.noloc, dist, test="Chisq")#significant
```
Location has no effect on distance anymore, X2(1) = 2.213, p = .137.
