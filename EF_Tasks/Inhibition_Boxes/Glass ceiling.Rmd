...
#Full model
res<-glmer(Trial_Correct ~ z.age.midtesting + z.Trial_nr + TestingLocation + z.age.midtesting:z.Trial_nr + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
```

Comparison against null model
```{r}
null<-glmer(Trial_Correct ~ 1 + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
...
```{r}
res.nointer<-glmer(Trial_Correct ~ z.age.midtesting + z.Trial_nr + TestingLocation + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
...
New full model
```{r}
res<-glmer(Trial_Correct ~ z.age.midtesting + z.Trial_nr + TestingLocation + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
anova(null, res, test="Chisq")
```
...
```{r}
res.notest<-glmer(Trial_Correct ~ z.age.midtesting + z.Trial_nr + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
...
```{r}
res<-glmer(Trial_Correct ~ z.age.midtesting + Trial.Nr + (1+Trial.Nr|ID), data=GC.valid, family = binomial)
null<-glmer(Trial_Correct ~ 1 + (1+Trial.Nr|ID), data=GC.valid, family = binomial)
anova(null, res, test="Chisq")
summary(res)
```
...
```{r}
res.notrial<-glmer(Trial_Correct ~ z.age.midtesting + (1+Trial.Nr|ID), data=GC.valid, family = binomial)
...
**Effect of age**
```{r}
res.noage<-glmer(Trial_Correct ~ Trial.Nr + (1+Trial.Nr|ID), data=GC.valid, family = binomial)
...
res<-glmer(Trial_Correct ~ z.age.midtesting + z.Trial_nr + (1+z.Trial_nr|ID), data=GC.valid, family = binomial)
