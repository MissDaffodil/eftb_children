...
```{r}
Foil.valid.T1.PC <- subset(Foil.valid.T1, PC1 !="NA")
res <- lm(Trial_correct ~ PC1 + PC2, data = Foil.valid.T1.PC)
null <- lm(Trial_correct ~ 1, data = Foil.valid.T1.PC)

anova(null, res, test="Chisq")
```
PC1 and PC2 cannot predict performance on the foil task, X2(2) = 0.895, p = .136.