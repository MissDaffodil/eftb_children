...
## Can the Principal components predict the Tool use score?
```{r}
Tool.valid.PC <- subset(Tool.valid, PC1 != "NA")
Tool.valid.PC$Score <- as.factor(Tool.valid.PC$Score)

#we z-transform age and trial number, as we are including them into an interaction
Tool.valid.PC$z.age=as.vector(scale(Tool.valid.PC$MidtestingAgeMonths))#transform age to mean of 0 and SD of 1
mean(Tool.valid.PC$z.age)#check whether it has worked
sd(Tool.valid.PC$z.age)

Tool.valid.PC$Gender

res <- polr(Score ~ z.age + PC1 + PC2, data = Tool.valid.PC)
null <- polr(Score ~ 1, data = Tool.valid.PC)

anova(null, res, test="Chisq")
```
Age and the PC scores cannot predict the tool use score.